...
-----
#
# Conf name  conf time                deadline      notification
#
EGSR2010     2010/06/28-2010/06/30    2010/04/09    2010/05/14
HPG2010      2010/06/25-2010/06/27    2010/04/09    2010/05/08
SIGGASIA     2010/12/15-2010/12/18    2010/05/11    <unknown>
-----
Copyright (C) 2010 Yamauchi, Hitoshi.
HELP
end
...
class Draw_conf_schedule
  ...
  def parse_one_line(_line)
    begin
      ...
      cdata = _line.split
      if cdata.length < 4 then
        raise RuntimeError.new("conference data line doesn't have enough information\n")
      end

      conf_name              = cdata[0]
      conf_start_end_date    = cdata[1].split("-")
      conf_deadline_date     = cdata[2]
      conf_notification_date = cdata[3]
...
HEADER
  ...
  # read a file
  #
  def read(_infilename, _outfilename)
    begin
      # open the input
      if _infilename != "-" then
...
dcs.read($OPT_INFILE, $OPT_OUTFILE)
